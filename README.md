...
| 폴더 | 책의 부 | 내용 |
|---|---|---|
| `dataset/` | 공용 | 개발셋 60건 + 검증셋 60건 (부서 라벨 포함) |
| `ch01/` | 1부 | LLM의 한계 실측 |
| `ch02/` | 2부 | 첫 판단 호출 |
...
| `ch13/` | 13부 | 세무 상담 에이전트(LangChain), 프롬프트 대 판단 모델 비교 |
...
`dataset/inquiries.py` 는 **개발셋**입니다. 후보 정의를 다듬고 임계값을 고르는 데 씁니다.
`dataset/holdout.py` 는 **검증셋**입니다. 그렇게 정한 설정의 성능을 보고할 때만 씁니다.

책의 초고는 이 둘을 나누지 않아 게이트 신뢰도를 1.000으로 보고했는데,
검증셋에서 다시 재니 0.946이었습니다. **5.4퍼센트포인트가 낙관 편향의 크기입니다.**

검증셋을 보고 설정을 바꾸면 그 순간 검증셋이 아니게 됩니다.
